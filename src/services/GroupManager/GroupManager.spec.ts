...
describe('SummariesManager', () => {
  test('add() should add a summary to the manager', () => {
    ...
    manager.add(EnumTimeLimit['30_MINUTES'], 'Test summary');
    expect(manager.hasValidSummary(EnumTimeLimit['30_MINUTES'])).toBe(true);
  });

  test('delete() should delete a summary from the manager', () => {
    const manager = new SummariesManager();
    manager.add(EnumTimeLimit['30_MINUTES'], 'Test summary');
    manager.delete(EnumTimeLimit['30_MINUTES']);
    expect(manager.hasValidSummary(EnumTimeLimit['30_MINUTES'])).toBe(false);
  });
...
describe('GroupManager', () => {
  test('addSummary() should add a summary to the group', () => {
    ...
    manager.addSummary('group1', EnumTimeLimit['30_MINUTES'], 'Test summary');
    const test = manager.getSummaryById('group1', EnumTimeLimit['30_MINUTES']);
  ...
  test('addSummary() should not add a summary if a valid summary with the same time limit exists', () => {
    ...
    manager.addSummary('group1', EnumTimeLimit['30_MINUTES'], 'Test summary');
    expect(
      manager.getSummaryById('group1', EnumTimeLimit['30_MINUTES']),
    ).not.toBeNull();

    // Wait for the summary to expire
    jest.advanceTimersByTime(31 * 60 * 1000);

    manager.addSummary('group1', EnumTimeLimit['30_MINUTES'], 'New summary');
    expect(
      manager.getSummaryById('group1', EnumTimeLimit['30_MINUTES']),
    ).not.toBeNull();
  });
